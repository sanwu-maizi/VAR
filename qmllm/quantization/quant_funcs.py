...
def pseudo_quantize_tensor(tensor, n_bits=8, zero_point=True, q_group_size=-1, per_tensor=False, inplace=False):
    ...
    #     print("警告：scales 包含 NaN 或 Inf，替换为 1e-5")
    #     scales = torch.nan_to_num(scales, nan=1e-5, posinf=1e-5, neginf=1e-5)

    # print("tensor",tensor,"isnan",torch.isnan(tensor).sum())
