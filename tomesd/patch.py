...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    block_idx = tome_info["block_idx"]
    
    b,s,c = x.shape
    
    depth = int(math.sqrt(s))
    
    if depth>=16 and block_idx%2==1:
        w = depth
        h = depth
        ...
        m, u, prune_indices = merge.bipartite_soft_matching_random2d(x, w, h, args["sx"], args["sy"], r, 
                                                      no_rand=not use_rand, generator=args["generator"], return_indices=True)
        tome_info["kv_prune_indices"] = prune_indices
    else:
        m, u = (merge.do_nothing, merge.do_nothing)
        tome_info["kv_prune_indices"] = None  # 无修剪时设为 None
...
#             return x
        ...
        def forward(self, x, cond_BD, attn_bias):   # C: embed_dim, D: cond_dim
            # print(x.shape)
            m_a, m_m, u_a, u_m = compute_merge(x, self._tome_info)
            # print(m_a,u_a)
            if self.shared_aln:
            ...
            x = x + self.drop_path(u_a(self.attn(m_a(self.ln_wo_grad(x).mul(scale1.add(1)).add_(shift1)), attn_bias=attn_bias )).mul_(gamma1))
            x = x + self.drop_path(u_m(self.ffn(m_m(self.ln_wo_grad(x).mul(scale2.add(1)).add_(shift2)))).mul(gamma2)) # this mul(gamma2) cannot be in-placed when FusedMLP is used
            return x
...
def hook_tome_model(model: torch.nn.Module):
    ...
    def hook(module, args):
        ...
        module._tome_info["depth"] = (args[0])
        return None
...
def apply_patch(
    ...
    diffusion_model._tome_info = {
        ...
        "hooks": [],
        "depth": 0,
        "block_idx": None,  # 当前 block 的索引，稍后在遍历中设置
        "kv_prune_indices": None,  # 需要删去的 KV 索引，稍后计算
        "args": {
            "ratio": ratio,
    ...
    for _, module in diffusion_model.named_modules():
        ...
        if isinstance_str(module, "AdaLNSelfAttn"):
            make_tome_block_fn = make_tome_block
            module.__class__ = make_tome_block_fn(module.__class__)
            module._tome_info = diffusion_model._tome_info.copy()  # 每个 block 独立副本
            module._tome_info["block_idx"] = block_counter  # 设置当前 block 的索引
            block_counter += 1
            
            # 设置默认的 kv_prune_indices（稍后由 compute_merge 更新）
            module._tome_info["kv_prune_indices"] = None
